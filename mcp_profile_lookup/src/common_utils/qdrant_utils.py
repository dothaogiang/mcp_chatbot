...
from common_utils.constants import DENSE_VECTOR_NAME, SPARSE_VECTOR_NAME
...
def ensure_collections():
    """Tạo collection nếu chưa tồn tại. Gọi 1 lần khi khởi động server / sync job."""
    client = get_client()
    vectors_config = {
    ...
    for collection in (config_object.COLLECTION_ARCHIVES, config_object.COLLECTION_CHUNKS):
...
def upsert_archive(archive_id: str, dense_vec: list, sparse_vec: dict, payload: dict):
    client = get_client()
    client.upsert(
        collection_name=config_object.COLLECTION_ARCHIVES,
        points=[
            models.PointStruct(
...
def upsert_chunks(archive_id: str, file_url: str, chunks: list, dense_vecs: list, sparse_vecs: list, extra_payload: dict):
    ...
    if points:
        client.upsert(collection_name=config_object.COLLECTION_CHUNKS, points=points)


def delete_chunks_by_file(archive_id: str, file_url: str):
    """Xóa chunk cũ của 1 file trước khi ingest lại (khi file bị thay đổi nội dung)."""
    client = get_client()
    client.delete(
        collection_name=config_object.COLLECTION_CHUNKS,
...
def hybrid_search(collection: str, dense_vec: list, sparse_vec: dict, limit: int, query_filter=None):
    """
    Hybrid search dense + sparse, fusion bằng RRF (Reciprocal Rank Fusion).
    query_filter (nếu có) được áp vào cả 2 nhánh prefetch -> dùng để giới
    hạn get_profile_detail chỉ tìm trong đúng 1 archive_id.
    """
    client = get_client()
